...
import { ArrowRight, Handshake, UsersRound, UserSquare } from 'lucide-react';
...
const PartnersPage: NextPage = () => {
  ...
  const logoPath = (name: string) => `/images/logos/${name}-logo.png`; // Using defined paths
          ...
          <Card className="perspex-card p-8 md:p-12">
            <div className="flex flex-wrap items-center justify-center gap-x-16 gap-y-8">
              {techPartners.map(partner => (
                <Image key={partner} src={logoPath(partner)} alt={`${partner} Logo`} width={150} height={50} className="opacity-80 hover:opacity-100 transition-opacity" data-ai-hint={`${partner} logo`} />
              ))}
            </div>
          </Card>
        </ScrollAnimatedSection>
